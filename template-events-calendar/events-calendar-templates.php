...
if (!class_exists('EventsCalendarTemplates')) {
    class EventsCalendarTemplates {
		...
		public function ect_styles(){
			...
			wp_register_style('ect-list-styles', ECT_PLUGIN_URL . 'assets/css/ect-list-view.css',null, null,'all' );
   		...
   		public function ect_shortcodes($atts){
			...
			if ($all_events) {		
				...
				if(!empty($event_link)){
					...
					$event_content.='<span class="event-link">' . $event_link_html . "</span>";
				}

				if(!empty($event_email)){
					$event_email_html = '<a href="mailto:' . $event_email . '" class="far fa-envelope" target="_blank"></a>';
					$event_content.='<span class="event-link">' . $event_email_html . "</span>";
				}
